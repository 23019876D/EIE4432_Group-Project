...
import { fetch_match } from './matchdb.js';
import { fetch_user_transactions } from './transactiondb.js';
...
route.get('/transactions', async (req, res) => {
  ...
  try {
    const userTransactions = await fetch_user_transactions(userId);
    const transactionsWithMatches = await Promise.all(
      userTransactions.map(async (transaction) => {
        const match = await fetch_match(transaction.matchid);
        return {
          ...transaction,
          match: match || null,
        };
    ...
    res.json({ status: 'success', transactions: transactionsWithMatches });
  } catch (error) {
    console.error('Error fetching transactions and matches:', error);
    res.status(500).json({ status: 'failed', message: 'Error fetching transactions' });
  }
});
